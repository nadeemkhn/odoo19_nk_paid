...
class DeliveryCarrier(models.Model):
    ...
    def leopards_rate_shipment(self, order):
        ...
        try:
            # Weight: context order_weight (delivery wizard), then order weight fields if present, then from lines
            weight = self.env.context.get('order_weight')
            if weight is None or weight == 0.0:
                if 'shipping_weight' in order._fields:
                    weight = order.shipping_weight or 0.0
                elif 'weight' in order._fields:
