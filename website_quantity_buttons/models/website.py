...
class Website(models.Model):
    ...
    website_qty_button_enabled = fields.Boolean(
        ...
        help="Enable quantity quick-select buttons for this website.",
    )
    website_qty_button_values = fields.Char(
        string="Website Quantity Values",
        default="1,2,3",
        help="Comma-separated positive quantities (max 6 buttons). Example: 1,2,3",
    )
    ...
    def website_get_qty_button_values(self):
        self.ensure_one()
        if not self.website_qty_button_enabled:
            return []
