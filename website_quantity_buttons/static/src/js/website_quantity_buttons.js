...
const pendingQuantityByProduct = new Map();
...
function findSalePriceNode(priceContainer) {
    return priceContainer?.querySelector(
...
function applyInstantPriceScale(root, previousQuantity, nextQuantity) {
    ...
    if (salePriceNode && salePriceValue !== null) {
        ...
        setMonetaryText(salePriceNode, priceToStr(estimatedTotal));
    }
    ...
    if (defaultStrikeNode && defaultStrikeValue !== null) {
        ...
        setMonetaryText(defaultStrikeNode, priceToStr(estimatedStrike));
    }
    ...
    if (compareStrikeNode && compareStrikeValue !== null) {
        ...
        setMonetaryText(compareStrikeNode, priceToStr(estimatedCompare));
    }
}
...
function updateDisplayedPrices(root, quantityInfo, quantity = 1) {
    ...
    const salePriceNode = findSalePriceNode(priceContainer);
    if (salePriceNode) {
        setMonetaryText(salePriceNode, priceToStr(totalPrice, precision));
    }
    ...
    if (hasComparePrice) {
        if (compareStrikeWrapper && compareStrikeNode) {
            ...
            setMonetaryText(compareStrikeNode, priceToStr(
        ...
        if (defaultStrikeWrapper && defaultStrikeNode) {
            ...
            setMonetaryText(defaultStrikeNode, priceToStr(
...
function rememberPendingQuantity(button) {
    ...
    const quantity = parseQuantity(
        card.dataset.selectedQtyButton
        || card.querySelector(".o_qty_choice.active")?.dataset.qty
        || card.querySelector(".o_qty_choice")?.dataset.qty,
        1,
    );
    ...
    if (Number.isInteger(productTemplateId)) {
        pendingQuantityByProduct.set(`t:${productTemplateId}`, quantity);
    }
    if (Number.isInteger(productId)) {
        pendingQuantityByProduct.set(`p:${productId}`, quantity);
    }
}

function getPendingQuantity(productTemplateId, productId) {
    const templateKey = Number.isInteger(productTemplateId) ? `t:${productTemplateId}` : null;
    const productKey = Number.isInteger(productId) ? `p:${productId}` : null;

    const quantity = (productKey && pendingQuantityByProduct.get(productKey))
        || (templateKey && pendingQuantityByProduct.get(templateKey));

    if (productKey) {
        pendingQuantityByProduct.delete(productKey);
    }
    if (templateKey) {
        pendingQuantityByProduct.delete(templateKey);
    }
...
patch(CartService.prototype, {
    async add(product, options = {}) {
        if (!product.quantity) {
            const pendingQuantity = getPendingQuantity(product.productTemplateId, product.productId);
            if (pendingQuantity) {
                product = { ...product, quantity: pendingQuantity };
            }
        }
        return super.add(product, options);
    },
});
...
document.addEventListener("click", (ev) => {
    ...
    const snippetAddButton = ev.target.closest(".o_carousel_product_card .js_add_cart");
    if (snippetAddButton) {
        rememberPendingQuantity(snippetAddButton);
    }
}, true);
